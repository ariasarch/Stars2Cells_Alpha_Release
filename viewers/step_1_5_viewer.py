...
class Step1_5Viewer(QDialog):
    ...
    def update_recommendation(self, data, optimal_threshold, peak_quality, peak_match_rate, has_quality_curves, has_match_counts):
        ...
        if has_quality_curves:
            ...
            text += '<b>⚠️ Old Format:</b> No quality curves available.<br>'
        ...
        text += f'<b>√N Scaling:</b> C = {C:.4f} (R² = {r_squared:.3f})<br>'
        text += f'Formula: τ = {C:.4f} × √N<br><br>'
        
        text += '<b>Example Predictions:</b><br>'
        text += f'• N = 100 neurons → τ = {C * np.sqrt(100):.4f}<br>'
        text += f'• N = 300 neurons → τ = {C * np.sqrt(300):.4f}<br>'
        text += f'• N = 500 neurons → τ = {C * np.sqrt(500):.4f}<br><br>'
    ...
    def plot_calibration_fit(self, data):
        ...
        ax.scatter(np.sqrt(N_sorted), tau_sorted, s=100, alpha=0.6, 
                  label='Measured τ', color='steelblue')
        ...
        ax.plot(sqrt_N_range, tau_fit, 'r--', linewidth=2,
                label=f'τ = {C:.4f} × √N (R² = {r_squared:.3f})')
        
        ax.set_xlabel('√N (√neurons)', fontsize=12)
        ax.set_ylabel('Optimal Threshold τ', fontsize=12)
        ax.set_title(f'√N Scaling Calibration – Animal {self.current_animal}',
